    ...
    if (contentType.includes("application/json")) {
      ...
      if (body.mode === "GENERATE_COPY") {
        try {
          ...
          // productPrompt → person+product scene direction (Gemini Flash excels at visual prompts)
          // NOTE: sceneAction is NOT here — it's generated by OpenAI per variant to ensure
          // each variant gets a unique, diverse scene. Gemini Flash only generates ONE prompt
          // which would be duplicated across all variants, killing diversity.
          const IMAGE_BRIEF_FIELDS = ["productPrompt"] as const;
