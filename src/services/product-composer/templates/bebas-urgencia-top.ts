...
export function buildBebasUrgenciaTopLayout(
  ...
  const MARGIN = Math.round(CW * 0.01); // ~11px

  // ── Headline sizing ──────────────────────────────────────────────────────
  // 0% margin each side → text block = 100% of canvas width, full bleed
  // Maximum wrap width lets the most words fit per line; font shrinks to fit 2 lines max.
  const HL_W = CW; // 1080px full width
  const HL_X = 0;
  const HL_MAX_FS = Math.round(CW * 0.085); // ~92px — slightly smaller to fit more words per line
  const HL_MIN_FS = Math.round(CW * 0.04); // ~43px floor (lower to accommodate long text)
  ...
  return {
    ...
    confidence: 1,
    rationale:
      "bebas-urgencia-top — Full-bleed urgency scene, single oversized Bebas Neue headline centered (84% width, 2 lines max), dark solid overlay. Logo auto top-left. No product.",
    warnings: [
